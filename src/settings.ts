import { App, PluginSettingTab, type SettingDefinitionItem, type SettingGroupItem } from "obsidian";
		...
		return [
				...
				items: this.plugin.settings.rules.map(
					(rule, index): SettingGroupItem => ({
						...
						render: (setting) => {
							...
							const arrows = setting.settingEl.createDiv({ cls: "afe-rule-arrows" });
							...
							const body = setting.settingEl.createDiv({ cls: "afe-rule-body" });
			...
			},
			{
				...
				render: (setting) => {
					setting
						.addButton((btn) =>
							btn.setButtonText("+ directory").onClick(() => void (async () => {
						...
						.addButton((btn) =>
							btn.setButtonText("+ content").onClick(() => void (async () => {
								this.plugin.settings.rules.push({
									type: "content",
									label: "",
									pattern: "",
						...
						.addButton((btn) =>
							btn.setButtonText("+ directory & content").onClick(() => void (async () => {
								this.plugin.settings.rules.push({
									type: "both",
									directory: "",
									label: "",
